...
get.prototype.pre = function (event, context) {
    ...
    let duf_subtier_id = event.app.authorizer.dufSubtierId;    
...
get.prototype.chunk = async function (chunk) {
    ...
    if (invoices.items && invoices.items[0] && invoices.items[0].items) {
        ...
        for (const item of invoices.items[0].items) {
            ...
            var erp = event.requestContext.authorizer.erp?.toUpperCase();
            ...
            item.transactions.forEach(function (transaction_item, idx) {
                ...
                var type_card = "";
                var terminal = "";
                let payment_auth_code = "";
                let transaction_reference = "";
                let credit_card_number = '';
                if(!credit_card_number && transaction_item?.additional_information?.adyen_card_bin && transaction_item?.additional_information?.cc_last_4){
                    // If adyen_card_bin is present we are displaying first six digit and last four digit of the credit card number
                    credit_card_number = transaction_item?.additional_information?.adyen_card_bin +  process.env.CREDIT_CARD_INFIX + transaction_item?.additional_information?.cc_last_4;
                }else if (transaction_item?.additional_information?.cc_last_4){
                    // As we getting only the last four digit of the credit card. 
                    // we are formatting the credit card number to 16 digits by adding upto 12 'X' character.                   
                    credit_card_number = process.env.CREDIT_CARD_PREFIX + transaction_item?.additional_information?.cc_last_4;
                }

                const paymentMethodName = transaction_item.additional_information.method
                    ? transaction_item.additional_information.method
                    : transaction_item.additional_information.method_title;
                
                // Transaction reference refers to psp reference of the credit card.
                transaction_reference = transaction_item?.additional_information?.pspReference;
                           
                switch (paymentMethodName) {
                    case "adyen_cc":
                    case "Adyen CreditCard":
                        cc_type = transaction_item.additional_information.payment_method; 
                        payment_auth_code = transaction_item.additional_information.adyen_auth_code;                        
                        break;                    
                    case "authnetcim":
                    case "CC":
                        cc_type = transaction_item.additional_information.card_type;                        
                        payment_auth_code = transaction_item.txn_id;
                        break;
                    case "tns_hpf":
                        ...
                        break;
                    case "adyen_hpp":
                        cc_type = extractAdyenHostedPaymentPagesCreditCardType(transaction_item.additional_information);
                        payment_auth_code = transaction_item.additional_information.adyen_auth_code;                        
                        break;
                    case "mercadopago_adbpayment_cc":
                        cc_type = extractMarcadoHostedPaymentPagesCreditCardType(transaction_item.additional_information);
                        credit_card_number = transaction_item?.additional_information?.card_number;
                        payment_auth_code = transaction_item.txn_id;
                        break;
                    case "checkmo":
                    case "free":
                ...
                item.payment.push({
                    ...
                    "terminal_id": terminal, "transaction_reference": transaction_reference,"authorization_code": payment_auth_code,
                    "authorization_date": order_date,"amount": total_paid.toFixed(2)+"",
            ...
            if(item.customer_balance_invoiced>0){                 
                item.payment.push({ "type_card": process.env.VOUCHER_CC_TYPE, "card_number": "XXXXXXXX", "terminal_id":"", "transaction_reference": process.env.VOUCHER_PSP_REFERENCE,"authorization_code":"", "authorization_date":"","amount":  item.customer_balance_invoiced});
            }
...
function getInvoiceTotalV2(item){
    ...
    return parseFloat(item.subtotal_invoiced) + parseFloat(item.shipping_amount) + parseFloat(item.tax_invoiced) - check_discount_amount_val;
}
...
const extractAdyenHostedPaymentPagesCreditCardType = (paymentInformation) => {
    const paymentMethod = paymentInformation.payment_method || '';
    ...
    return cardTypeMapping[paymentMethod] || '';
};


const extractMarcadoHostedPaymentPagesCreditCardType = (paymentInformation) => {
    const paymentMethod = paymentInformation.card_type || '';  
